...
# ── 持久快取（API 失敗時的 fallback）──
_DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "data")
_PE_CACHE_FILE = os.path.join(_DATA_DIR, "fundamental_pe_cache.json")
_REV_CACHE_FILE = os.path.join(_DATA_DIR, "fundamental_rev_cache.json")

# API 重試冷卻：失敗後 5 分鐘內不重試，避免每輪都卡 timeout
_PE_RETRY_COOLDOWN = 300
_pe_last_api_attempt: float = 0.0
_REV_RETRY_COOLDOWN = 300
...
def fetch_twse_revenue_all() -> Dict[str, dict]:
    ...
    """
    global _rev_last_api_attempt
    now = time.time()
    if _rev_cache and now - _rev_cache.get("time", 0) < REV_CACHE_TTL:
        return _rev_cache["data"]

    # API 失敗冷卻中，直接回傳舊快取
    if now - _rev_last_api_attempt < _REV_RETRY_COOLDOWN and _rev_cache.get("data"):
        return _rev_cache["data"]
    _rev_last_api_attempt = now

    url = "https://openapi.twse.com.tw/v1/opendata/t187ap05_L"
    try:
        resp = legacy_get(url, timeout=10, headers={"User-Agent": "Mozilla/5.0"})
        data = resp.json()
        result = {}
        for row in data:
            code = row.get("公司代號")
...
def fetch_twse_pe_all() -> Dict[str, dict]:
    ...
    """
    global _pe_last_api_attempt
    now = time.time()
    if _pe_cache and now - _pe_cache.get("time", 0) < PE_CACHE_TTL:
        return _pe_cache["data"]

    # API 失敗冷卻中，直接回傳舊快取
    if now - _pe_last_api_attempt < _PE_RETRY_COOLDOWN and _pe_cache.get("data"):
        return _pe_cache["data"]
    _pe_last_api_attempt = now
    ...
    try:
        resp = legacy_get(openapi_url, timeout=15, headers={"User-Agent": "Mozilla/5.0"})
        data = resp.json()
    ...
    if _pe_cache.get("data"):
        logger.warning("TWSE P/E API 失敗，使用本地快取（%d 筆）", len(_pe_cache["data"]))
    else:
        logger.error("TWSE P/E 無任何快取可用")
