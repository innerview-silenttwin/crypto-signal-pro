...
SECTOR_STOCKS = {
    ...
    "其他": {
        ...
        # 主動 ETF（2026-06-17 加入，6mo 領先大盤、樣本 ≥ 6 個月的保守版 6 檔）
        # 詳見 BEAT_ETFS（active_etf.py）+ memory/project_active_etf_us.md
        "00988A.TW": "主動統一全球創新",   # 6mo +129%、領先大盤 +64.7pp（持股偏美股）
        "00990A.TW": "主動元大AI新經濟",   # 6mo +109%、領先 +44.1pp（持股偏美股）
        "00981A.TW": "主動統一台股增長",   # 6mo  +95%、領先 +29.9pp
        "00982A.TW": "主動野村優選成長",   # 6mo  +77%、領先 +12.5pp（name 待二次驗證）
        "00985A.TW": "主動野村台灣50",     # 6mo  +74%、領先 +9.6pp
        "00980A.TW": "主動野村臺灣優選",   # 6mo  +68%、領先 +3.7pp
    },
}
        ...
        *,
        ...
        if trade_type == "BUY":
            if self.state["balance"] < 100:
                # per-symbol 診斷：用 info 級讓 prod log 隨時看得到真實狀況（哪檔因餘額不足被跳）。
                logger.info(
                    "[%s] %s BUY SKIP: balance < 100 (balance=%.0f)",
                    self.sector_name, symbol, self.state["balance"],
                )
                return False
